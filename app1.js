...
app.get('/LoggedIN/news', checkAuthenticated, (req, res) => {
  res.render('news', { username: req.user.username})
})
...
app.get('/LoggedIN/admin/:name', checkAuthenticated, (req, res)=>{
  ...
  users.forEach(function(user){
    ...
    if (name === StoredUsername ){
      ...
      if(length == 0){
        res.render("admin",{username: user.username , prefernces: user.bio, email: user.email, password: user.password, Posts: [] });
      } else {
